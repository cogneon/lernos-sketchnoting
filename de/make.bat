...
pandoc metadata.yaml -s --resource-path="./src" ./src/%filename%.md -o %filename%.docx
echo -- Word document created

REM Create Web Version (html)
echo -- Create HTML document
pandoc metadata.yaml -s --resource-path="./src" --toc ./src/%filename%.md -o %filename%.html
echo -- HTML document created

REM Create PDF Version (pdf)
echo -- Create PDF document
pandoc metadata.yaml --from markdown --resource-path="./src" --template sketchnotes --number-sections -V lang=de-de ./src/%filename%.md -o %filename%.pdf 
...
pandoc metadata.yaml -s --resource-path="./src" --epub-cover-image=src/images/ebook-cover.png ./src/%filename%.md -o %filename%.epub
